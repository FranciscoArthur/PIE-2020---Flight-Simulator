...
Outputs : Atmospheric parameters at the plane altitude :
    [0] : Air temperature (Celsius)
    [1] : Air temperature (Kelvin)
    [2] : Air pressure (Pa)
    [3] : Air density (kg/m3)
    [4] : Air speed of sound (m/s)
    [5] : Local gravitational acceleration (m/s2)
    [6] : Air dynamic viscosity (kg/m/s)
    [7] : Air kinematic viscosity (m2/s)
    [8] : Air thermal conductivity (W/m/K)
...
def atmospheric_parameters_fct(h):   # h = altitude (m)
    if not isinstance(h, int):
        raise ValueError("The input 'altitude' must be an integer")
    
    H = geom2geop_height(h)                       # Geopotential altitude [m]
    ...
    grav_acceleration = grav_accel(h)
    dyn_viscosity = dynamic_viscosity(T)
    kin_viscosity = dyn_viscosity/dens
    th_conductivity = thermal_conductivity(T)

    return ([altitude, temp_Celsius, temp_Kelvin, press, dens, speed_sound, grav_acceleration, dyn_viscosity, kin_viscosity, th_conductivity])
...
# PAIRWISE: Function tu build up pairs from a list
